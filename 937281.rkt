...
(define get-between
  (lambda (start finish)
    (filter even? (drop(iota (+ (* finish -1) 1))  (* start -1)))))
...
(define evens-between-kernel
  (lambda (start finish)
    (cond
      ...
      [(> start 0)
       (filter even? (drop(iota (+ finish 1))start))]
      [(< start 0)
...
; Examples/Tests:
;Tests of the evens-between procedure > Preconditions are not met
;Preconditions are not met
;FAILURE
;name:       check-exn
;location:   937281.rkt:222:11
;params:     #<procedure:conjoined>
;#<procedure:...sktop/937281.rkt:254:19>
;message:    "No exception raised"
;7 success(es) 1 failure(s) 0 error(s) 8 test(s) run 1
...
; Time Log:
;   Date        Start   Finish  Elapsed Activity
;   3/2         12:55    2:00   figuring out word length
;   3/4         9:30     10:00   writing the conditions
;   3/6         9:45     10:00   writing character compare
; Time Spent: 
; 1 hour 50 min
; Citations:
...
; Time Log:
;   Date        Start   Finish  Elapsed Activity
;   3/02        2:00     2:55   Testing the code out 
; Time Spent: 
; 55 min
; Citations:
